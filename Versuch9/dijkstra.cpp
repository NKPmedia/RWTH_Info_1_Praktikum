...
std::vector<Street*> Dijkstra::search(const AbstractMap &map, QString start, QString target)
{
    ...
    while (yellow.empty() == false)
    {
        ...
        std::vector<Street*> streets = map.get_street_list(edge_to_process_data.city);
    ...
    std::vector<Street*> shortest_path;
...
void Dijkstra::drawWay(std::vector<Street*> &way, QGraphicsScene &scene)
{
